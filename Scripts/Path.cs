...
namespace TDTK
{
    ...
    public class Path : MonoBehaviour
    {
        ...
        public static List<Path> GetAllStartingPath()
        ...
        public static Path GetNextShortestPath(Path path, bool bypass = false)
        {       //get the nearest path to destination from the path nextPath
            List<int> list = new List<int>();
            for (int i = 0; i < completePathList.Count; i++)
            {           //get all potential path
        ...
        public void UpdateDistToEnd()
        {   //the result is only correct if nextShortestIdx is true, run UpdateDistToFinalDestOnAllPath
            cachedDistToEnd = CalculateDistToEnd();
        }
        public float CalculateDistToEnd(float dist = 0)
        ...
        public Path GetNextShortestPath() { return nextPath.Count == 0 ? null : nextPath[nextShortestIdx]; }
		...
		*/
        ...
        public void UpdatePlatformPath(BuildPlatform platform, UnitTower tower = null)
        {
            ...
            for (int i = 0; i < wpSecList.Count; i++)
            {
                ...
                {
                    if (tower != null)
                    {   //if the tower isn't built in the path, there's no need to update the path
                        bool inPath = false;
                    ...
                    {   //if(wpSecList[i].wpList.Count==0){
                        ...
                        for (int n = 0; n < creepOnPath.Count; n++)
                        {
                            ...
                            if (creepOnPath[n].wpIdx == 0)
                            {               //fix added on 18Jan2018
